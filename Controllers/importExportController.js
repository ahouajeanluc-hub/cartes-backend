...
// 🛠️ CLASSES UTILITAIRES (inchangées)
class ImportResult {
...
// 🎯 SERVICE PRINCIPAL - VERSION MONGODB CORRIGÉE
class CarteImportExportService {
  /**
   * Import d'un fichier Excel - VERSION MONGODB CORRIGÉE
   */
  static async importExcel(req, res) {
    console.time('⏱️ Import Excel');
    console.log('🚀 DEBUT IMPORT - Version MongoDB');
    ...
    try {
      ...
      // Traitement avec transaction MongoDB
      const result = new ImportResult(importBatchID);
      await session.withTransaction(async () => {
        await this.processImport(worksheet, headers, result, req, importBatchID, session);
      });
  ...
  /**
   * Processus d'import principal - MONGODB
   */
  static async processImport(worksheet, headers, result, req, importBatchID, session) {
    ...
    for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
      try {
        ...
        await this.processSingleRow(rowData, rowNumber, result, req, importBatchID, session);
  ...
  /**
   * Traitement d'une seule ligne - MONGODB
   */
  static async processSingleRow(rowData, rowNumber, result, req, importBatchID, session) {
    try {
      // Mapping des données
      const mappedData = {
      ...
      // Vérifier les doublons dans MongoDB
      const duplicateCheck = await getDB().collection('cartes').countDocuments({
        NOM: cleanedData.NOM,
      ...
      // Insertion avec ImportBatchID dans MongoDB
      const carteDocument = {
        ...cleanedData,
        importBatchID: importBatchID,
        created_at: new Date(),
      ...
      const resultInsert = await getDB().collection('cartes').insertOne(
        carteDocument,
        { session }
      );

      const carteId = resultInsert.insertedId;
      result.imported++;
  ...
  // 🔥 MÉTHODES D'EXPORT - ADAPTÉES MONGODB
  static async exportAll(req, res) {
    try {
      const cartes = await getDB().collection('cartes')
        .find({})
        .sort({ _id: 1 })
        .toArray();
      ...
      const normalizedData = this.normalizeMongoData(cartes);
      ...
      await journalController.logAction({
        ...
        tableName: 'Cartes',
        ip: req.ip,
        details: `Export complet - ${cartes.length} cartes - Fichier: ${filename}`
      });
  ...
  static async exportSearchResults(req, res) {
    try {
      ...
      Object.entries(filterMap).forEach(([key, field]) => {
        if (req.query[key] && req.query[key].trim() !== '') {
          ...
          if (field.includes('DATE')) {
            // Pour les dates, recherche exacte
            query[field] = value;
          } else {
      ...
      console.log('📝 Requête MongoDB générée:', query);
      const cartes = await getDB().collection('cartes')
        .find(query)
        .sort({ _id: 1 })
        .toArray();
      ...
      await journalController.logAction({
        ...
        tableName: 'Cartes',
        ip: req.ip,
        details: `Export résultats recherche - ${cartes.length} cartes - Fichier: ${filename} - Critères: ${JSON.stringify(req.query)}`
      });
  ...
  static normalizeMongoData(documents) {
